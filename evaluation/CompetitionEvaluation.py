...
from evaluation.IgnoranceScore import ensemble_ignorance_score_xskillscore
from evaluation.IntervalScore import mean_interval_score_xskillscore
from utils.utilities import remove_duplicated_indexes, match_actual_prediction_index
...
def structure_data(
    ...
    odim = dict(xobserved.sizes)
    pdim = dict(xpred.sizes)
    if "member" in pdim:
        del pdim["member"]
    assert (
        odim == pdim
