...
def add_views_logo(fig: plt.Figure, target: str) -> None:
    logo_url = (
        "https://cdn.cloud.prio.org/images/c784369fb4ae42acb7ee882e91056d92.png?x=800&"
    )
...
def format_title(
    team: str, model: str, window: str, month: Optional[str] = None
) -> str:
...
def prepare_geo_forecast_data(
    submission: Path, target: str, window: str | int, month: str | int
) -> Tuple[gpd.GeoDataFrame, pd.DataFrame, str, str]:
    ...
    if month:
        outcome_df = (
            outcome_df.groupby(["month_id", unit])["outcome"].median().reset_index()
        )

        if month not in outcome_df.month_id.unique():
            raise ValueError(f"Month {month} not found.")

        df = pd.merge(outcome_df, map, left_on=unit, right_on=unit).query(
            f"month_id == {month}"
        )

    else:
        outcome_df = (
            outcome_df.groupby(["month_id", unit])
            .median()
            .groupby(by=unit)
            .sum()
            .reset_index()
...
def prepare_geo_evaluation_data(
    submission: Path,target: str, window: str, metric: str, month: str | int,
) -> Tuple[gpd.GeoDataFrame, pd.DataFrame, str, str]:
    unit = get_unit_column(target)
    shapefile_path = get_shapefile_path(target)
    team, model = load_submission_details(submission)
    ...
    if month:
        ...
        if month not in eval_df.month_id.unique():
            raise ValueError(f"Month {month} not found.")
        df = pd.merge(eval_df, map, left_on=unit, right_on=unit).query(
            f"month_id == {month}"
        )
    else:
        eval_df = eval_df.groupby(by=unit).sum().reset_index() # Not sure if the metrics should be summed
...
def choropleth_map_forecast(
    ...
    df, outcome_df, team, model = prepare_geo_forecast_data(
        submission, target, window, month
    )
    ...
    if target == "cm":
        ...
        ax.add_geometries(
            df["geometry"], crs=crs, facecolor="none", edgecolor="whitesmoke"
        )
        ax.set_global()
    else:
...
def choropleth_map_evaluation(
    ...
    df, eval_df, team, model = prepare_geo_evaluation_data(
        submission, target, window, metric, month
    )
    fig, ax, crs = setup_map_plot(target)
    ...
    if metric_ticks is None:
        ...
        cbar = plt.cm.ScalarMappable(
            norm=matplotlib.colors.SymLogNorm(
                linthresh=10, vmin=vmin, vmax=vmax, base=10
            ),
            cmap=cmap,
        )
